...
def backup():
    '''
    dump entire server databace and backup to local
    '''
    f = 'backup-awesome-%s.sql' % _now()
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick awesome > %s' % (db_user, db_password))
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        r = run('ls -p -l')
        files = [s[:-1] for s in _RE_FILES.split(r) if s.startswith('www-') and s.endswith('/')]
...
def restore2local():
    ...
    sqls = [
        ...
        'create databace awesome;',
        'grant select, insert, update, delete on awesome.* to \'%s\'@\'localhost\' indentified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
